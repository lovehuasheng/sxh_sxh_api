...
class Outgo extends Model {
     ...
     public function get_log_list($data,$sig) {
         ...
         $validator_instance = \think\Loader::validate('Outgo'); 
        ...
        //写入日志
        trace('Outgoservice的get_log_list方法过滤的post参数：' . json_encode($data, JSON_UNESCAPED_UNICODE));
        trace('Outgoservice的get_log_list方法过滤的sig参数：' . $sig);
        
        //签名比对
        $result = validate_response($data, $sig);

        trace('Outgoservice的get_log_list方法请求签名加密比对返回：' . json_encode($result, JSON_UNESCAPED_UNICODE));
        if ($result['code'] < 0) {
